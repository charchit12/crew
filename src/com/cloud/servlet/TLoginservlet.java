...
@WebServlet(name = "TLogin", urlPatterns = {"/TLogin"})
public class TLoginservlet extends HttpServlet {

    private static final long serialVersionUID = 1L;
    static Logger logger = Logger.getLogger(TLoginservlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
    {
        ...
        else 
        {
            ...
            try 
            {
               ps = con.prepareStatement("select email, fname,password from trainer where email='" + email + "' and password='" + password + "'");
               rs = ps.executeQuery();
                if (rs != null) 
                {
                    ...
                    {
                        request.setAttribute("action", "TLogin");
                        request.setAttribute("date", dateFormat.format(date));
                        ...
                        RequestDispatcher rd = getServletContext().getRequestDispatcher("/TLogin.jsp");
                        rd.include(request, response);
                    }
                } 
                else 
                {
                    ...
                    request.setAttribute("msg", "<font color=red>User Name or Password is wrong.</font>");
                    request.setAttribute("passwordVal", "<font color=red>User Name or Password is wrong.</font>");
                    RequestDispatcher rd = getServletContext().getRequestDispatcher("/TLogin");
                    rd.include(request, response);
                }
